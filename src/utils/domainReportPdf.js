...
function watermarkCommand() {
  const centerX = PAGE_WIDTH / 2;
  const centerY = PAGE_HEIGHT / 2;
  const angleRad = (-35 * Math.PI) / 180;
  const a = Math.cos(angleRad).toFixed(4);
  const b = Math.sin(angleRad).toFixed(4);
  const c = (-Math.sin(angleRad)).toFixed(4);
  const d = Math.cos(angleRad).toFixed(4);

  return (
    `q /GS1 gs 0.00 0.65 0.45 rg ` +
    `${a} ${b} ${c} ${d} ${centerX.toFixed(2)} ${centerY.toFixed(2)} cm ` +
    `BT /F2 52 Tf -200 -18 Td (${pdfEscape(WATERMARK_TEXT)}) Tj ET Q`
  );
}

function rightAlignX(text, size, rightEdge) {
  return rightEdge - measureText(text, size);
}

function centerAlignX(text, size, centerX) {
  return centerX - measureText(text, size) / 2;
}
...
class PdfBuilder {
  ...
  addPage() {
    ...
    this.commands = [watermarkCommand()];
    this.y = TOP_MARGIN;
  }
...
function serializePdf(pages, title) {
  ...
  const watermarkGsId = addObject("<< /Type /ExtGState /ca 0.08 /CA 0.08 >>");
  const pageIds = [];
  ...
  pages.forEach((commands) => {
    ...
    const pageId = addObject(
      `<< /Type /Page /Parent ${pagesId} 0 R /MediaBox [0 0 ${PAGE_WIDTH} ${PAGE_HEIGHT}] /Resources << /Font << /F1 ${fontRegularId} 0 R /F2 ${fontBoldId} 0 R >> /ExtGState << /GS1 ${watermarkGsId} 0 R >> >> /Contents ${contentId} 0 R >>`
    );
    pageIds.push(pageId);
  });
...
export function createDomainSecurityReportPdf({ domain, vulnerabilities }) {
  ...
  pdf.text("PENTESTRADAR", MARGIN_X, PAGE_HEIGHT - 50, { size: 24, font: "F2", color: "0.00 0.84 0.56" });
  pdf.text("Security Assessment Report", MARGIN_X, PAGE_HEIGHT - 78, {
    size: 10,
    font: "F1",
    color: "0.39 0.45 0.55",
  });

  const reportLabel = "SECURITY REPORT";
  pdf.text(reportLabel, rightAlignX(reportLabel, 18, PAGE_WIDTH - MARGIN_X), PAGE_HEIGHT - 50, {
    size: 18,
    font: "F2",
    color: "0.06 0.09 0.16",
  });

  pdf.line(MARGIN_X, PAGE_HEIGHT - 105, PAGE_WIDTH - MARGIN_X, PAGE_HEIGHT - 105, "0.80 0.84 0.88", 1);

  pdf.text(cleanText(domain.domain), MARGIN_X, PAGE_HEIGHT - 130, {
    size: 16,
    font: "F2",
    color: "0.06 0.09 0.16",
  });
  ...
  const footerLine = `Generated by PentestRadar (${WATERMARK_TEXT}) - Confidential Security Report`;
  pdf.text(footerLine, centerAlignX(footerLine, 9, MARGIN_X + CONTENT_WIDTH / 2), PAGE_HEIGHT - pdf.y, {
    size: 9,
    font: "F1",
    color: "0.58 0.63 0.69",
  });
